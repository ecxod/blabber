...
* show only one avatar and username for several consecutive messages of a contact and some minor UI improvements (blabber.im)
* add prefence to disable persistent room creation (blabber.im)
* some more database optimizations (blabber.im)
* add long-press context menu on list of conversations (blabber.im)
* fix WEBRTC compatibility issues (blabber.im)
* improve removing tracking parameters from shared urls (blabber.im)
* show green dot, if user is using the app (blabber.im)
* show progress in % of video compression, file up- and download in chat and chat overview (blabber.im)
...
* some bugfixes
* hard coded a provider domain as standard server
