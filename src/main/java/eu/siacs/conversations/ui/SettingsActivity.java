...
public class SettingsActivity extends XmppActivity implements OnSharedPreferenceChangeListener {
    ...
    public static final String SENTRY_DSN = SentryManager.PREFERENCE_DSN;
    ...
    public void onStart() {
        ...
        if (quickAction != null && removeVoice) {
            ...
            int index = entryValues.indexOf("voice");
        ...
        final PreferenceScreen ExpertPreferenceScreen = (PreferenceScreen) mSettingsFragment.findPreference("expert");
    ...
    private void configureSentryPreference() {
        final EditTextPreference sentryPreference =
                (EditTextPreference) mSettingsFragment.findPreference(SENTRY_DSN);
        if (sentryPreference == null) {
            return;
        }
        updateSentryPreferenceSummary(sentryPreference);
        sentryPreference.setOnPreferenceChangeListener((preference, newValue) -> {
            final String dsn = SentryManager.normalize(String.valueOf(newValue));
            if (!dsn.isEmpty() && !SentryManager.isValidDsn(dsn)) {
                displayToast(getString(R.string.pref_sentry_dsn_invalid));
                return false;
            }
            return true;
        });
    ...
    private void updateSentryPreferenceSummary(final Preference preference) {
        final String dsn = PreferenceManager.getDefaultSharedPreferences(this)
                .getString(SENTRY_DSN, "");
        preference.setSummary(SentryManager.isValidDsn(dsn)
                ? R.string.pref_sentry_dsn_configured
                : R.string.pref_sentry_dsn_disabled);
    }
    ...
    public void onSharedPreferenceChanged(SharedPreferences preferences, String name) {
        ...
        } else if (name.equals(SENTRY_DSN)) {
            SentryManager.applyConfiguration(getApplicationContext());
            final Preference sentryPreference = mSettingsFragment.findPreference(SENTRY_DSN);
            if (sentryPreference != null) {
                updateSentryPreferenceSummary(sentryPreference);
            }
        }
